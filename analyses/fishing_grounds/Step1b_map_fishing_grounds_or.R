...
g <- ggplot(ports) +
  ...
  # Crop
  coord_sf(xlim = c(-125, -122), ylim = c(42, 46.4)) +
  # Theme
  theme_bw() + my_theme
g

# Export
ggsave(g, filename=file.path(plotdir, "FigSX_or_fishing_ports.png"), 
...
ggplot() +
  ...
  geom_sf(data=usa, fill="grey90", color="white", lwd=0.3, inherit.aes = F) +
  # Crop
  coord_sf(xlim = c(-126, -122), ylim = c(40, 47)) +
  # Theme
  theme_bw()
...
# Export
saveRDS(ports, file=file.path(outdir, "ports_or.Rds"))
saveRDS(contours_all1, file=file.path(outdir, "fishing_grounds_or.Rds"))
