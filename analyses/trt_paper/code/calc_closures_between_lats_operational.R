...
out_status <- c("Out-of-season")
quality_status <-  c("Body condition delay", 
                     "Body condition/domoic acid delay",                   
                    "Domoic acid delay",
                    "Evisceration order")
whale_status <- c("Whale entanglement closure",
                  ...
                  "33% gear reduction",                              
                  "50% gear reduction")
merged_statuses <- c(open_status, out_status, quality_status, whale_status)
...
stats <- purrr::map_df(row_ids, function(x){
  ...
  sdata <- data %>% 
    ...
    summarize(ndays=n_distinct(date),
              ...
              latdays_out=sum(status %in% out_status) * 0.01,
              latdays_quality=sum(status %in% quality_status) * 0.01,
              latdays_whale=sum(status %in% whale_status) * 0.01,) %>% 
    ungroup() %>% 
    mutate(latdays_open_prop=latdays_open/latdays_tot,
           latdays_out_prop=latdays_out/latdays_tot,
           latdays_quality_prop=latdays_quality/latdays_tot,
           latdays_whale_prop=latdays_whale/latdays_tot) %>% 
    # Add row id
    mutate(row_id=x)
...
stats_out <- stats %>% 
  ...
  # Check sum
  mutate(prop_check=latdays_open_prop+latdays_out_prop+latdays_quality_prop+latdays_whale_prop)
...
outfile <- ifelse(version=="real",
                  "percent_of_zones_under_restriction_by_week_and_zone_real_operational.csv",
                  "percent_of_zones_under_restriction_by_week_and_zone_jameal_operational.csv")
